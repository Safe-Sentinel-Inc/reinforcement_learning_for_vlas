...
class PaligemmaTokenizer:
    ...
    def tokenize(self, prompt: str, state: np.ndarray | None = None, advantage: bool | None = None, pred_value: bool = False, stage: int | None = None) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if state is not None:
            # This is the Pi05 format, where the state is part of the discrete language input.
            discretized_state = np.digitize(state, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
            state_str = " ".join(map(str, discretized_state)) # Convert state array to space-separated string
            if pred_value:
                full_prompt = f"Task: {cleaned_text}, State: {state_str}, Stage: {stage};\nValue: " # For value function.
            else:
                if advantage is not None:
                    advantage_str = "Positive" if advantage else "Negative"
                    full_prompt = f"Task: {cleaned_text}, State: {state_str}, Advantage: {advantage_str};\nAction: " # For advantage-conditioned policy.
                else:
                    full_prompt = f"Task: {cleaned_text}, State: {state_str};\nAction: " # For standard policy without advantage conditioning.
