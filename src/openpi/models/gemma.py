...
class Config:
    ...
    # Gemma 3 style QK-norm (RMSNorm on Q/K before attention scoring).
    use_qk_norm: bool = False
    # If set, use this scalar for attention scaling instead of head_dim.
    query_pre_attn_scalar: float | None = None
    # Gemma 3 uses pre+post norms per sublayer (4 norms per layer).
    use_post_norm: bool = False
    lora_configs: dict[str, lora.LoRAConfig] = dataclasses.field(default_factory=dict)

#添加gemma270M
Variant = Literal["dummy", "gemma_300m", "gemma_300m_lora", "gemma_2b", "gemma_2b_lora", "gemma_270m"]
...
def get_config(variant: Variant) -> Config:
    ...
    #新增270M配置，参数量与google/gemma-3-270m相当，架构也基本一致
    if variant == "gemma_270m":
        # ~270M params, matches google/gemma-3-270m (Gemma 3) architecture.
        return Config(
            width=640,
            depth=18,
            mlp_dim=2048,
...
class Attention(nn.Module):
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        ...
        # QK-norm (Gemma 3): normalize Q and K BEFORE RoPE.
        # Uses the same (1 + scale) convention as HuggingFace Gemma3RMSNorm.
        # QK-norm实现
        if self.configs[0].use_qk_norm:
            q = RMSNorm(name="q_norm")(q, None)[0]
            k = RMSNorm(name="k_norm")(k, None)[0]
        ...
        # Attention scaling.
        # 注意力缩放
        if self.configs[0].query_pre_attn_scalar is not None:
            q *= self.configs[0].query_pre_attn_scalar ** -0.5
        else:
...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True):  # noqa: FBT002
        ...
        #实现post_norm
        use_post_norm = self.configs[0].use_post_norm
        ...
        # Gemma 3: post-attention norm (applied before residual addition).
        if use_post_norm:
            post_attn = [
                RMSNorm(name=_name("post_attention_norm", i))(y, None)[0]
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                # Gemma 3: post-FFW norm (applied before residual addition).
                if use_post_norm:
                    x = RMSNorm(name=_name("post_ffw_norm", i))(x, None)[0]  # noqa: PLW2901
            out.append(x)
            gates.append(gate if x is not None else None)
